...
export default function Page() {
	...
	const sendTestEvents = useCallback(async () => {
		...
		const testEvents = [
			{ event_name: "app_started", properties: { source: "verify-page" } },
			{
				...
				properties: { feature: "dashboard", source: "verify-page" },
			},
			{
				event_name: "app_started",
				properties: { source: "verify-page", session: 2 },
			},
		];
		...
		for (const evt of testEvents) {
			try {
				...
				if (res.ok) {
					...
					addLog(
						`Failed to send "${evt.event_name}": ${data.error || res.status}`,
						false,
					);
				}
			} catch (e: any) {
				addLog(
					`Network error sending "${evt.event_name}": ${e.message}`,
					false,
				);
			}
		}
		...
		addLog(
			`Sent ${sent}/${testEvents.length} test events`,
			sent === testEvents.length,
		);
	}, [addLog]);
	...
	const verifyEvents = useCallback(async () => {
		...
		try {
			...
			setStats(data);
			const total =
				data.by_event?.reduce((sum: number, e: any) => sum + e.count, 0) ?? 0;
			if (total > 0) {
				setVerifyStatus("ok");
